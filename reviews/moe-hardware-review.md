...
검증을 진행했습니다. 아래는 그 과정에서 나온 **판단과 새로 발견한 사실**만 남긴
것입니다 — 구체적 수치·표는 반영이 끝난 `build-guide.md` v16 쪽이 원본이므로 여기서
다시 옮기지 않습니다(중복 방지, 8-4 참고).

### 8-1. 코딩 능력 — "단순"과 "에이전틱"은 다른 문제였습니다

벤치마크를 HumanEval/LiveCodeBench(단일 함수·스크립트)와 SWE-bench Verified(레포
탐색·멀티스텝 이슈 해결)로 나눠보면, **"간단한 코딩"은 12B든 26B-A4B든 이미 잘
처리합니다** — 사용자의 직감이 맞았습니다. 26B-A4B로 바뀌어도 이 영역은 최소 현행
유지, 근소하게 개선됩니다. **"에이전틱 코딩"만 26B-A4B로도 못 메우는 격차**이고, 이건
Qwen3.6-35B-A3B가 유일한 해법입니다(수치 표: `build-guide.md` 4장). → **B(26B-A4B)를
채택하고, 에이전틱 코딩은 build-guide.md v16의 부록 A로 조건부 보류.**

### 8-2. Quant/오프로드 재검증 — B의 실행 가능성이 처음 추정보다 좋습니다

**QAT 버전이 따로 있고 훨씬 가볍다는 게 재검증 포인트입니다** —
`unsloth/gemma-4-26B-A4B-it-qat-GGUF`의 `UD-Q4_K_XL`이 비-QAT 버전보다 가벼워, 3장에서
쓴 "14-18GB" 추정치의 하단에 정확히 맞아떨어졌습니다. 그 결과 SFF 가용 VRAM 대비
오프로드 필요분이 Qwen의 1/3 이하로 줄어, 4-2에서 지적한 "SFF의 WSL RAM 24GB는 MoE
오프로드 하한선 아래"라는 문제가 **B에는 사실상 적용되지 않는다**는 게 확인됐습니다.
`--n-cpu-moe`가 mainline llama.cpp에서 26B-A4B를 정식 지원함(Qwen 전용 기능이 아님)과
Gemma 4 계열 최대 컨텍스트 262,144(Hermes 64K 하한 통과)도 함께 확인했습니다.
(정확한 GB·컨텍스트 수치: `build-guide.md` 3-4)
...
기준, "not planned"로 종료 — 자체 해결 가능성 낮음). SFF의 RTX 4070 SUPER(Ada Lovelace)
에서 재현된다는 보고는 없지만, 안전하다고 확인된 것도 아닙니다. 원래 B를 고른 이유 중
하나("비전 유지")를 직접 건드리는 리스크라 가볍게 넘기지 않았습니다.
(대응 절차·테스트 순서: `build-guide.md` 3-4, 6-3, 12-3 0번, 13장 7번)

### 8-4. 최종 확정

메인 모델을 Gemma4-12B → **Gemma4-26B-A4B**(QAT)로 교체, Qwen3.6-35B-A3B는 조건부
보류(RAM 28GB+ 확보 & 실제 에이전틱 코딩 수요 확인 시 재검토, 부록 A). **컨텍스트,
`--n-cpu-moe`, `--repeat-penalty` 등 실제 설정값은 `build-guide.md` v16이 단일
소스입니다** — 이 문서와 값이 어긋나면 build-guide.md를 따르세요.
